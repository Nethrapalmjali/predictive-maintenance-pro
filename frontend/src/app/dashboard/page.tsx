...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      const activeSession = supabaseSession || mockSession;
    ...
    const {
      ...
      setSession(session);
      if (!session && !localStorage.getItem('mock-session')) {
          router.push("/login");
      }
    });
  ...
  useEffect(() => {
    // Auto-load first machine data if empty
    if (session && reports.length === 0 && !isLoading) {
      handleSimulate({
        scenario: "normal",
        machine_id: "MACH-001",
  ...
  }, [session, reports.length, isLoading, handleSimulate]);
                ...
                <div className="animate-slideInRight">
                  ...
                  {activeTab === "charts" && (
                    <SensorCharts
                      ...
                      rawReadings={selectedReport.readings as any}
                    />
                  )}
                </div>
              )}
